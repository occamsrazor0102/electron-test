"""Lightweight HTTP API server for the BEET 2.0 desktop app.

Uses only Python standard-library modules so no extra dependencies are needed.
The server wraps the detection pipeline and exposes four endpoints:

    GET  /health          — liveness probe + available detector info
    POST /analyze         — analyze a single text
    POST /batch           — analyze a list of texts
    GET  /config          — return active configuration / profile info

Response bodies are always JSON.  CORS headers are included so the React
dev server (localhost:5173) can call the API during development.
"""
...
from llm_detector import __version__ as _VERSION  # type: ignore[import-untyped]

# ---------------------------------------------------------------------------
# Mapping helpers
# ---------------------------------------------------------------------------

# Base p_llm estimates per determination band (midpoint of each band).
...
# Width of the confidence interval in p_llm space (half-width).
_INTERVAL_HALF = 0.12


def _pipeline_to_report(result: dict[str, Any], profile: str = "default") -> dict[str, Any]:
    """Convert the raw pipeline dict to the BeetReport schema expected by the GUI."""
    det = result.get("determination", "UNCERTAIN")
    # Normalise legacy REVIEW label
    if det == "REVIEW":
        det = "UNCERTAIN"

    p_llm = _DET_P_LLM.get(det, 0.5)
    # Use calibrated_confidence (0–1 certainty in the label) to narrow/widen CI.
    certainty = float(result.get("calibrated_confidence") or result.get("confidence") or 0.5)
    half = _INTERVAL_HALF * (1.0 - certainty * 0.5)

    # Derive top signals from channel details
    ...
    # Fallback: look at raw signal flags in the result dict
    if not top_signals:
        signal_fields = (
            "preamble_severity",
    ...
    # Build layer_results from channel data
    layer_results = []
...
class BeetHandler(BaseHTTPRequestHandler):
    """HTTP request handler for the BEET API."""

    # Suppress default request log (server uses its own logger)
    ...
    def _handle_health(self) -> None:
        from llm_detector.compat import HAS_SEMANTIC, HAS_PERPLEXITY  # lazy import
    ...
    def _handle_analyze(self) -> None:
        from llm_detector.pipeline import analyze_prompt  # lazy import
    ...
    def _handle_batch(self) -> None:
        ...
        for sub in submissions:
            ...
            if not text.strip():
                continue
            try:
                result = analyze_prompt(text, task_id=sub.get("id", ""), occupation=occupation)
                reports.append(_pipeline_to_report(result, profile))
            except Exception:  # noqa: BLE001
                pass  # Skip failed items rather than failing the whole batch
...
# ---------------------------------------------------------------------------
# Server start / stop helpers (called by the Electron main process via CLI)
# ---------------------------------------------------------------------------

def run_server(host: str = "127.0.0.1", port: int = 8000) -> None:
    """Start the BEET HTTP server (blocking).  Typically called by ``llm-detector serve``."""
...
if __name__ == "__main__":
    ...
    _p = _ap.ArgumentParser(description="BEET HTTP API server")
